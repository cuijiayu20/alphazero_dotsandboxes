...
class DotsAndBoxesGame(Game):
    ...
    def display(board,action_copy):
        ...
        for i in range(n):
            ...
            if i < n - 1:
                for j in range(n):
                    if (board[i + n][j] == 1 or board[i + n][j] == 2):
                        print(" |  ",end='')
                    else:
                        print('{:02d}'.format(30 + b), end='   ')
                    b = b + 1
                print(" ")
        ...
        # lock = threading.Lock()  #
        # lock.acquire()  # Get the lock before creating a thread.py
        # t = threading.Thread(target=GUI.create_board, args=(action_copy[-1],))
        # t.start()
        # t.join()  # Wait for the thread.py to finish
        # lock.release()
