...
def run_pit():
    ...
    numMCTSSims = 50
    #n1 = NNetWrapper(g)
    #n1.load_checkpoint(os.path.join('..', 'pretrained_models', 'dotsandboxes', 'keras', '3x3'), 'best.pth.tar.index')
    args1 = dotdict({'numMCTSSims': numMCTSSims, 'cpuct': 1.0})
    #mcts1 = MCTS(g, n1, args1)
    #n1p = lambda x: np.argmax(mcts1.getActionProb(x, temp=0))

    #n2 = NNetWrapper(g)
    #n2.load_checkpoint(os.path.join('..', 'pretrained_models', 'dotsandboxes', 'keras', '3x3'), 'best.pth.tar.index')
    ...
    # p2 = grp2
    # arena = Arena.Arena(p1, p2, g, display=DotsAndBoxesGame.display)
    # oneWon, twoWon, draws = arena.playGames(1000, verbose=False)
    # print("oneWon: {}, twoWon: {}, draws: {}".format(oneWon, twoWon, draws))
